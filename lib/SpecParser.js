...
module.exports = class SpecParser {
  ...
  async parseInfo(input) {
    ...
    let templateDir = path.join(process.cwd(), infoDoc.template)
    let isRelative = await filesys.canAccess(templateDir)
    
    if (!isRelative) {
      
      // If not relative, try node_modules
      templateDir = path.join(process.cwd(), 'node_modules', infoDoc.template)
      let isNodeDir = await filesys.canAccess(templateDir)
      
      // If not a node module, fail
      if (!isNodeDir) {
        throw new Error(`Cannot file template '${infoDoc.template}'`)
      }
    }
    
    let templateResult = await this.validateTemplate(templateDir)
  ...
  async processVersion(filedir, filepath, doc) {
    ...
    if (doc.length > 1) {
      ...
      // If one doc, generate the meta doc
      metaDoc = {
        version: path.basename(filedir),
        base: '/',
        groups: Object.keys(doc[0])
      }
      dataDoc = doc[0]
    }
