...
module.exports = class SpecParser {
  ...
  async parseInfo(input) {
    ...
    try {
      ...
      let template = infoDoc.template || 'ocelot-template'
      ...
      if (!isRelative) {
        ...
        templateDir = path.join(process.cwd(), 'node_modules', template)
        let isNodeDir = await filesys.canAccess(templateDir)
        
        // If not a node module, fail
        if (!isNodeDir) {
          return result.addError(`Cannot file template '${template}'`)
        }
      }
