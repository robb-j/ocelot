...
;(async () => {
  ...
  if (program.watch) {
    startWatchServer(program.input, program.output)
  }
  else {
...
function startWatchServer(input, output) {
  
  // Create an express server
  let app = express()
  
  // On get, process & render endpoints
  let middleware = (req, res, next) => processEndpoints(input, output).then(next)
  app.use(middleware, express.static(output))
  
  // Listen on 8080
  console.log('Watch server started on localhost:8080')
  app.listen(8080)
  opn('http://localhost:8080')
}

/** Processes the endpoints in 'input' and render to 'output' */
async function processEndpoints(input, output) {
  ...
  await SpecRenderer.run(result.value, outputDir)
}
